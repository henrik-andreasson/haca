...
if [ "x$1" != "x" ] ; then
    ...
    echo "name,userid,serial,orgunit,org,country,profile,service_name,ca_name,validity_start,validity_end"
    exit
fi
...
for row in $(cat "${csvfile}") ; do
  ...
  crl_cdp=$(echo $row | cut -f11 -d\,)
  ocsp_url=$(echo $row | cut -f12 -d\,)
  iscomment=$(echo $row | grep "^#" )
  if [ "x$iscomment" != "x" ] ; then
    continue
  fi
  result=$(http --verify cacerts.pem  POST "${API_URL}/ca/generate" \
    "name=${name}" \
    "userid=${userid}" \
    "serial=${serial}" \
    "orgunit=${orgunit}" \
